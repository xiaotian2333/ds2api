...
func (h *Handler) listAccounts(w http.ResponseWriter, r *http.Request) {
	...
	for _, acc := range accounts[start:end] {
		...
		if muteInfo, ok := h.Store.AccountMuteStatus(acc.Identifier()); ok && muteInfo != nil {
			item["is_muted"] = muteInfo.Muted
			if muteInfo.MuteUntil > 0 {
				item["mute_until"] = muteInfo.MuteUntil
			}
		}
