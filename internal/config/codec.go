...
func (c Config) MarshalJSON() ([]byte, error) {
	...
	if c.SensitiveWords.Enabled || len(c.SensitiveWords.Patterns) > 0 || strings.TrimSpace(c.SensitiveWords.BlockMessage) != "" {
		m["sensitive_words"] = c.SensitiveWords
	}
...
func (c *Config) UnmarshalJSON(b []byte) error {
	...
	for k, v := range raw {
		...
		case "sensitive_words":
			if err := json.Unmarshal(v, &c.SensitiveWords); err != nil {
				return fmt.Errorf("invalid field %q: %w", k, err)
			}
		case "vercel":
...
func (c Config) Clone() Config {
	clone := Config{
		...
		SensitiveWords: SensitiveWordsConfig{
			Enabled:      c.SensitiveWords.Enabled,
			Patterns:     slices.Clone(c.SensitiveWords.Patterns),
			BlockMessage: c.SensitiveWords.BlockMessage,
		},
		Vercel:           c.Vercel,
